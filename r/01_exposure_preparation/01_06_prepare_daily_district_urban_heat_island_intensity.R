# ==============================================================================
# Script: 01_06_prepare_daily_district_urban_heat_island_intensity.R
# Original file: 00_UHI_vorbereitung.R
#
# Purpose:
#   Aggregates hourly urban heat island intensity raster data to daily metrics
#   and extracts district-level spatial summaries.
#
# Inputs:
#   - Hourly NetCDF files containing urban heat island intensity
...
ordner_pfad_nc <- "data/raw/hostrada/urban_heat_island_intensity"
pfad_shp       <- "data/raw/spatial/vg2500/vg2500"
output_ordner  <- "data/intermediate/exposure/daily_urban_heat_island_intensity"
...
if(length(dateien_liste) == 0) stop("Keine .nc Dateien gefunden! Pfad prüfen.")
...
jahre_zu_berechnen <- jahre_zu_berechnen <- alle_jahre[alle_jahre >= 2000 & alle_jahre <= 2022]
...
for (jahr in jahre_zu_berechnen) {
  ...
  file_name <- file.path(output_ordner, paste0("UHI_Kreise_Daily_", jahr, ".csv"))
  ...
  for (i in seq_along(tage_im_jahr)) {
    ...
    df_tag <- data.frame(
      ...
      UHI_Mean = as.vector(ex_mean[,1]),
      UHI_Max  = as.vector(ex_max[,1]),
      UHI_Min  = as.vector(ex_min[,1]),
      UHI_Q90  = as.vector(ex_q90[,1]),
      UHI_SD   = as.vector(ex_sd[,1])
    )
